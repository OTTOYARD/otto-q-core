...
**APPLY ORDER for the window after round 38's last pair (15:55 UTC + runtime), `pg_stat_activity` clear:** `0256` (trigger fix, `forces_recert=TRUE`) **then** `0257` (second floor + catalog pin, `forces_recert=FALSE`). Both carry their own pre-flight and md5 guards. Both still say `migration-version: PENDING` in their headers, which is the in-repo marker that they are drafted and unapplied.
