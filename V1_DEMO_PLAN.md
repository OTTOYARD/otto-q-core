...
- [x] Phase 1 — `p_policy` in the pair rig; shield held constant; one committed comparison *(2026-09-13 00:21 UTC: `0261` applied; `db/canons/ab_busy_day_424242_12t.md` — self-test byte-identical, otto_q/fifo reproduced byte-for-byte on re-run; OTTO-Q median turnaround 150 vs 180 min, safety identical)*
...
- [ ] Phase 2 — CP-SAT proposer certified; `h_prop` non-trivial in one pair *(2026-09-12: `bridge/` built and tested; `0259` precedence + `0260` fire ledger drafted, apply after round 39; then one live run, then the replay pair) **2026-09-13: two live runs, 90 rows submitted, 0 heard — six defects found and fixed or filed (L-58…L-62, 0262); the blocker is kernel-side: the frame does not carry reservations or charger state (L-61) and nothing holds the named stall. `demo/D3_RUNBOOK.md` §5.)*
