...
def _stall(sid, kind="dcfc", kw=150):
    return {"id": sid, "type": kind, "status": "available", "connector_max_kw": kw}
...
def test_flat_forecast_at_rush_hour_resolves_to_dispatch_rush():
    r = orchestrate(_forecast(), _frame(), CLASSES, site=SITE, now_hour=7)
...
def test_arrival_surge_at_rush_hour_resolves_to_demand_surge():
    ...
    r = orchestrate(fc, _frame(), CLASSES, site=SITE, now_hour=7)
...
def test_flat_forecast_at_overnight_hour_resolves_to_overnight():
    r = orchestrate(_forecast(), _frame(), CLASSES, site=SITE, now_hour=22)
...
def test_conductor_is_deterministic():
    fc = _forecast(mean_daily_arrivals=24.0,
                   per_hour_arrivals={7: 3.0, 8: 3.0, 9: 3.0})
    a = orchestrate(fc, _frame(), CLASSES, site=SITE, now_hour=7)
    b = orchestrate(fc, _frame(), CLASSES, site=SITE, now_hour=7)
...
def test_a_missing_soft_target_is_refused_not_guessed():
    ...
    try:
        orchestrate(_forecast(), _frame(), CLASSES, site=site, now_hour=7)
...
def test_a_malformed_forecast_raises():
    ...
    del fc["arrivals"]
    try:
        orchestrate(fc, _frame(), CLASSES, site=SITE, now_hour=7)
...
def test_a_bad_now_hour_is_refused():
    try:
        orchestrate(_forecast(), _frame(), CLASSES, site=SITE, now_hour=24)
