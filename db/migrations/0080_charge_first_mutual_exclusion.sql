...
--
-- EVIDENCE (run 6889a959, vehicle b9ef130e / Waymo-AV-008):
--   needs card: soc 93, target 100, soc_deficit_to_target=7, energy_urgency='due_soon',
--               must_do_now=['interior_deep_clean'], deferrable_now=['charge','sensor_calibration']
--   104 stall-assignment decisions for one vehicle (52 charger + 52 wash bay)
--   41 of 42 begin_charge commands refused 'superseded_by_newer_stall_command'
--   51 bookings interrupted 'bay_exit_before_planned_end'
--
-- ROOT CAUSE. decide_tick has two independent space-assignment loops with no mutual exclusion:
--   (3)  charge assignment   — charges ANY staged vehicle with soc < target_soc (charge anchor).
--   (4b) needs-card routing  — routes to wash/service bay when a must-do need has a space lane.
-- The (4b) firewall only skipped vehicles with 'charge' in must_do_now. This vehicle's charge is
-- DEFERRABLE (soc 93 > 80 floor, deficit 7 to 100 target), so the firewall passed it to the bay
-- while section (3) simultaneously routed it to a charger. Two commands, same vehicle, same tick ->
-- the confirm walk supersedes one -> the loser's need stays unmet -> both re-fire -> oscillation.
--
-- FIX. Enforce the documented doctrine (charge is the anchor leg, sequence_order 10: energy first,
-- then bays): (4b) now ALSO yields to section (3) when the vehicle still has a charge deficit to
-- target (COALESCE(soc_deficit_to_target,0) > 0), not just when charge is must_do_now. The existing
-- 0003 exception (interrupted bay work, energy not critical, no free charger) is unchanged.
--
-- NET EFFECT. One space per vehicle per tick: charge first, then bays, within the same atomic visit.
-- A vehicle below target charges to completion, THEN is routed to the bay on a later tick, exactly
-- as section (4)'s charge_complete_holding -> wash path already does.
