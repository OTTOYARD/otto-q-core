...
--
-- 0446  **The readiness gate's hold clock lived on the vehicle, not the run, so a stamp an earlier run left behind
...
--   reset between arms, so no verdict should move, but the recert floor is moving in this window anyway (0444,
--   0445) and this rides it rather than asserting that no atom reads `vehicles.config`.
...
BEGIN
  ...
  IF md5(v_src) <> 'd211e8250d75157016118b8be0a4b393' THEN
    RAISE EXCEPTION '0446 P1: twin.ottoq_sim_advance_service_flow md5 is %', md5(v_src);
  END IF;
  ...
  IF (SELECT count(*) FROM regexp_matches(v_src, 'held_since', 'g')) <> 2 THEN
    RAISE EXCEPTION '0446 P1: held_since is read or written somewhere this file does not cover';
  END IF;
END $$;
...
BEGIN
  ...
  v_new := replace(v_def,
    E'          v_since    := COALESCE((v_rec.config #>> ''{deploy_gate,held_since}'')::timestamptz, p_sim_clock_now);\n',
    E'          /* 0446 (G182): a hold stamp is this run''s or it is absent. An earlier run''s stamp made a fresh hold\n'
  ...
  v_new := replace(v_new,
    ...
    || E'                               ''held_since'', v_since, ''held_min'', round(v_held_min,1),\n',
    E'                          || jsonb_build_object(''deploy_gate'', jsonb_build_object(\n'
    || E'                               ''held_since'', v_since, ''run'', p_sim_run_id, ''held_min'', round(v_held_min,1),   /* 0446 */\n');
  IF v_new = v_def OR position('(v_rec.config #>> ''{deploy_gate,run}'') = p_sim_run_id::text' IN v_new) = 0
    ...
    RAISE EXCEPTION '0446: the service-flow splices did not both apply';
  END IF;
  EXECUTE v_new;
END $splice$;
...
BEGIN
  ...
  IF position('(v_rec.config #>> ''{deploy_gate,run}'') = p_sim_run_id::text' IN v_src) = 0
    ...
    RAISE EXCEPTION '0446 V1: the gate does not read and write the run-scoped stamp as the header says';
  END IF;
END $$;

-- ── LINEAGE ──
INSERT INTO public.ottoq_cert_lineage (name, forces_recert, note) VALUES
  ('0446_the_readiness_gates_hold_clock_lived_on_the_vehicle_so_an_old_runs_stamp_released_fresh_holds',
   true,
...
-- own elapsed sim time. Rollback: restore the service flow from ottoq_schema_snapshots label '0446_pre'.
