...
BEGIN
  ...
  IF v_jobs IS NOT NULL THEN
    RAISE EXCEPTION '0219 P-: certification jobs are still scheduled (%) — migrations wait for '
  ...
  IF v_pairs > 0 THEN
    RAISE EXCEPTION '0219 P-: a determinism pair is running right now';
  END IF;

  SELECT count(*) INTO v_runs FROM public.ottoq_sim_runs WHERE status = 'running';
  IF v_runs > 0 THEN
    RAISE EXCEPTION '0219 P-: % sim run(s) are in flight', v_runs;
  END IF;

  RAISE NOTICE '0219 P-: no certification scheduled, no pair running, no sim run in flight';
END $inflight$;
