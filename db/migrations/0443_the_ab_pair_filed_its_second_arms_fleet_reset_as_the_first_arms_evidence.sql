...
BEGIN
  ...
  IF v_pairs > 0 THEN RAISE EXCEPTION '0443 P0: a pair is running right now'; END IF;
...
BEGIN
  ...
  IF md5(v_src) <> 'a02d660122e25f46e649ce334f93c223' THEN RAISE EXCEPTION '0443 P1: ottoq_ab_pair md5 is %', md5(v_src); END IF;
  v_n := (length(v_src) - length(replace(v_src, c_open, ''))) / length(c_open);
  IF v_n <> 1 THEN RAISE EXCEPTION '0443 P1: the loop-open anchor matched % times', v_n; END IF;
  v_n := (length(v_src) - length(replace(v_src, c_close, ''))) / length(c_close);
  IF v_n <> 1 THEN RAISE EXCEPTION '0443 P1: the loop-close anchor matched % times', v_n; END IF;
  IF position('set_config(''ottoq.sim_run_id''' IN v_src) > 0 THEN RAISE EXCEPTION '0443 P1: the rig already resets the GUC'; END IF;
END $$;
...
BEGIN
  ...
  v_new := replace(v_def, c_open,
    ...
    || E'    PERFORM set_config(''ottoq.sim_run_id'', ''none'', true);  /* 0443 (0421 in this rig): the reset runs BEFORE this arm''s run exists */\n'
    || E'    PERFORM public.ottoq_tick_invariance_reset_fleet(p_depot, p_seed, p_sim_start);');
  v_new := replace(v_new, c_close,
       c_close || E'\n'
    || E'  -- 0443: stop_and_reset pins the tagging GUC to the run it tore down; what follows belongs to no arm\n'
  ...
  IF v_new = v_def OR (length(v_new) - length(replace(v_new, 'set_config(''ottoq.sim_run_id''', ''))) / length('set_config(''ottoq.sim_run_id''') <> 2 THEN
    RAISE EXCEPTION '0443: the splices did not both apply';
  END IF;
  EXECUTE v_new;
END $splice$;
...
BEGIN
  ...
  IF v_loop = 0 OR v_reset < v_loop OR v_reset > v_fleet OR v_after = 0 THEN
    RAISE EXCEPTION '0443 V1: the resets are not where 0421 and 0439 put them (loop %, reset %, fleet %, after-stop %)',
...
INSERT INTO public.ottoq_cert_lineage (name, forces_recert, note) VALUES
  ('0443_the_ab_pair_filed_its_second_arms_fleet_reset_as_the_first_arms_evidence',
   false,
...
-- forces_recert FALSE. Rollback: restore ottoq_ab_pair from ottoq_schema_snapshots label '0443_pre'.
