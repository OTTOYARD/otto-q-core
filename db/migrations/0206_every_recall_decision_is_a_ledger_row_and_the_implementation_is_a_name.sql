...
CREATE TABLE public.ottoq_recall_decisions (
  ...
  -- THE LEDGER'S ONLY TOTAL ORDER, and it is neither a clock nor a random id.
  -- decided_at is now(): transaction time, identical for every row a pair
  -- writes, so ordering by it is a tie broken by recall_id, which is a
  -- gen_random_uuid(). That is exactly the defect 0207 removed from the
  -- command walks, and this file's own A2 caught it here on the first run
  -- ("the ledger names naive_threshold_v1 after the swap" -- it did not; the
  -- ordering picked a random row). decision_seq is insertion order, which the
  -- deterministic decide path makes a function of the run.
  decision_seq       bigint GENERATED BY DEFAULT AS IDENTITY,
...
BEGIN
  ...
  BEGIN
    ...
    END IF;
    SELECT implementation INTO v_impl FROM public.ottoq_recall_decisions WHERE sim_run_id = v_run ORDER BY decision_seq DESC LIMIT 1;   -- issuance order, not the clock
...
-- =====================================================================
-- APPLIED 2026-09-07 18:27:44 UTC (1:27 PM CT) -- one transaction as
-- postgres, sent directly, SECOND attempt. Verified after:
--
--   lineage row                       present, 18:27:44.030, forces_recert TRUE
--   ottoq_recall_decisions            present, append-only trigger, FK NO ACTION
--   ottoq_recall_implementations      2 rows (naive_threshold_v1 active,
--                                     fixed_window_dummy parked)
--   policy catalog                    3 recall_* parameters
--   ottoq_evaluate_return_need        0c463ada -> 53018872b12d8032f8728851dff719e7
--   naive_threshold_v1, renamed back  0c463ada1588296a31ec1761d16a83d4 -- the
--                                     ladder is the pinned evaluator, byte for byte
--   ottoq_determinism_pair            47b5ac77 -> 788ed4b7c7517d868e95f42be7151a59
--   recert floor                      18:18:01 (0204) -> 2026-09-07 18:27:44
--   grid column canon_rcl             f981b17b, 38 ledger rows from A3's pair
--
-- THE FIRST ATTEMPT FAILED, AND THE FAILURE IS THE FINDING
-- ---------------------------------------------------------------------
-- A2 raised: "0206 A2 FAILED: the ledger names naive_threshold_v1 after
-- the swap". The swap had worked -- the wrapper returned evidence naming
-- fixed_window_dummy, and that check passed one line earlier. What failed
-- was the assertion's own READ of the ledger:
--
--     ORDER BY decided_at DESC, recall_id DESC LIMIT 1
--
-- decided_at defaults to now(), which is TRANSACTION time and therefore
-- identical for every row the probe wrote. So the sort was a tie on the
-- first key, broken by recall_id -- a gen_random_uuid(). The "most recent"
-- row was chosen at random, and it chose a naive one.
--
-- That is the SAME DEFECT CLASS 0207 removed from ottoq_react_to_refusals
-- and the three walks in ottoq_sim_confirm_commands one day earlier: an
-- order that looks like time but is a constant inside a transaction, with
-- a random id underneath it. The ledger was built with no total order of
-- its own. The fix is 0207's: ottoq_recall_decisions.decision_seq, a
-- bigint identity, and the assertion now orders on it.
--
-- Nothing from the failed attempt survived: verified after the rollback
-- that both tables were absent, no lineage row, no new functions, no
-- catalog rows, and ottoq_evaluate_return_need still at its 0c463ada pin.
--
-- WHAT ROUND 22 MUST SHOW
--   1. No canon moves that is attributable to this migration. A1 proves
--      naive_threshold_v1 is the pinned ladder byte for byte and it is the
--      default implementation, so the decisions themselves cannot change.
--      h_evt moves on every column, but that is 0204's, by design.
--   2. Every arm carries h_rcl and the arms AGREE on it, on all six
--      columns. That is the gate for promoting h_rcl into v_equal, the
--      way round 21 was the gate for h_rule (0205).
