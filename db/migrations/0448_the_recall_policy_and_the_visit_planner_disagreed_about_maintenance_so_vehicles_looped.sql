...
BEGIN
  ...
  IF v_jobs IS NOT NULL THEN RAISE EXCEPTION '0448 P0: certification jobs are still scheduled (%)', v_jobs; END IF;
  ...
  IF v_pairs > 0 THEN RAISE EXCEPTION '0448 P0: a pair is running right now'; END IF;
  SELECT count(*) INTO v_runs FROM public.ottoq_sim_runs WHERE status IN ('running','paused');
  IF v_runs > 0 THEN RAISE EXCEPTION '0448 P0: % sim run(s) running/paused -- apply between runs', v_runs; END IF;
END $inflight$;
...
BEGIN
  ...
  IF md5(v_src) <> '098ade627348b08e58941eb510f49dae' THEN
    RAISE EXCEPTION '0448 P1: ottoq_recall_naive_threshold_v1 md5 is %', md5(v_src);
  END IF;
  ...
  IF v_n <> 1 THEN RAISE EXCEPTION '0448 P1: the v1 interval rung matched % times', v_n; END IF;
  ...
  IF md5(v_src) <> '47c44f2559c1ccc2fea15c49d1c916f0' THEN
    RAISE EXCEPTION '0448 P1: ottoq.ottoq_derive_visit_needs md5 is %', md5(v_src);
  END IF;
  v_a := E'      ''est_min'',v_pm_min,''concurrency'',''bay'',''requires_bay'',''service_bay'',''carryover_eligible'',true);\n  END IF;\n';
  v_n := (length(v_src) - length(replace(v_src, v_a, ''))) / length(v_a);
  IF v_n <> 1 THEN RAISE EXCEPTION '0448 P1: the mechanical_pm append matched % times', v_n; END IF;
  ...
  IF position(v_a IN v_src) < position('''svc'',''sensor_calibration''' IN v_src) THEN
    RAISE EXCEPTION '0448 P1: mechanical_pm is no longer appended after sensor_calibration';
  END IF;
  ...
  IF to_regprocedure('public.ottoq_recall_interval_scheduled_v1(uuid,uuid,timestamp with time zone,numeric,numeric)') IS NOT NULL
    ...
    RAISE EXCEPTION '0448 P1: implementation 3 already exists';
  END IF;
  IF (SELECT max_value FROM public.ottoq_policy_param_catalog WHERE param_key = 'recall_implementation_id') <> 2 THEN
    RAISE EXCEPTION '0448 P1: recall_implementation_id''s envelope is not 1..2';
  END IF;
END $$;
...
BEGIN
  IF EXISTS (SELECT 1 FROM public.ottoq_policy_params WHERE param_key = 'recall_implementation_id' AND param_value <> 1) THEN
    RAISE EXCEPTION '0448 P2: recall_implementation_id is set to something other than 1 somewhere';
  END IF;
  IF (SELECT prosrc FROM pg_proc WHERE oid = 'public.ottoq_recall_naive_threshold_v1(uuid,uuid,timestamp with time zone,numeric,numeric)'::regprocedure)
    ...
    RAISE EXCEPTION '0448 P2: an existing recall implementation already emits service_interval_overdue';
  END IF;
END $$;
...
SELECT '0448_pre', 'function', n.nspname, p.proname, pg_get_functiondef(p.oid), md5(pg_get_functiondef(p.oid))
...
BEGIN
  ...
  v_new := replace(v_new, c_rung,
         E'    /* 0448 (G181) interval_scheduled_v1: interval maintenance is SCHEDULED work. Below the hard limit it is not a\n'
  ...
  IF v_new = v_def OR position('ottoq_recall_interval_scheduled_v1(' IN v_new) = 0
    ...
    RAISE EXCEPTION '0448: interval_scheduled_v1 was not derived cleanly from v1';
  END IF;
  EXECUTE v_new;
END $v3$;

COMMENT ON FUNCTION public.ottoq_recall_interval_scheduled_v1(uuid,uuid,timestamp with time zone,numeric,numeric) IS
  '0448: implementation 3, interval_scheduled_v1. naive_threshold_v1 verbatim except the interval rung: PM and '
...
INSERT INTO public.ottoq_recall_implementations (impl_id, implementation, evaluator_function, status, note) VALUES
  (3, 'interval_scheduled_v1', 'ottoq_recall_interval_scheduled_v1', 'parked',
   '0448 (G181). naive_threshold_v1 with interval maintenance treated as scheduled work: no recall below '
...
BEGIN
  ...
  v_new := replace(v_def, c_anchor,
         c_anchor
      || E'  /* 0448 (G181): a vehicle recalled because its interval maintenance was hard-overdue gets that maintenance on\n'
  ...
  IF v_new = v_def OR position('''service_interval_overdue''' IN v_new) = 0 THEN
    RAISE EXCEPTION '0448: the visit promotion did not apply';
  END IF;
  EXECUTE v_new;
END $splice$;
...
UPDATE public.ottoq_policy_param_catalog
   SET max_value = 3,
       description = 'Which registered Recall Decision procedure decides (ottoq_recall_implementations.impl_id): '
                  ...
                  || '3 = interval_scheduled_v1 (0448), interval maintenance as scheduled work.'
...
VALUES ('recall_interval_hard_overdue_mult',
        'interval_scheduled_v1 (0448): a deployed vehicle is recalled for PM or calibration only at or over this multiple '
       ...
       ('recall_min_productive_min',
        'interval_scheduled_v1 (0448): minimum sim-minutes deployed before an overdue-maintenance recall may fire.',
...
VALUES ('0448', '11111111-1111-1111-1111-111111111111', 'recall_implementation_id', 1, 3, '{}'::jsonb, 'busy_day', 48,
...
BEGIN
  ...
  -- put v1's rung back into v3 in place of everything from the 0448 comment to its END IF, and demand v1 exactly
  v_back := regexp_replace(v3, '    /\* 0448 \(G181\) interval_scheduled_v1:.*?min_productive_min'', 120\)\); RETURN;\n    END IF;\n', c_rung);
  IF md5(v_back) <> md5(v1) THEN
    RAISE EXCEPTION '0448 V1: implementation 3 differs from v1 somewhere other than the interval rung';
  END IF;
  IF (SELECT provolatile FROM pg_proc WHERE oid = 'public.ottoq_recall_interval_scheduled_v1(uuid,uuid,timestamp with time zone,numeric,numeric)'::regprocedure)
    ...
    RAISE EXCEPTION '0448 V1: implementation 3 does not keep v1''s volatility';
  END IF;
END $$;
...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM public.ottoq_recall_implementations
    ...
    RAISE EXCEPTION '0448 V2: implementation 3 is not registered as parked';
  END IF;
  IF (SELECT max_value FROM public.ottoq_policy_param_catalog WHERE param_key = 'recall_implementation_id') <> 3
    ...
    RAISE EXCEPTION '0448 V2: the envelope or the new dials are wrong';
  END IF;
  IF (SELECT count(*) FROM public.ottoq_dial_experiments
    ...
    RAISE EXCEPTION '0448 V2: the recall experiment is not registered exactly once';
  END IF;
  -- the incumbent is untouched: every stored recall_implementation_id is still 1
  IF EXISTS (SELECT 1 FROM public.ottoq_policy_params WHERE param_key = 'recall_implementation_id' AND param_value <> 1) THEN
    RAISE EXCEPTION '0448 V2: the incumbent moved';
  END IF;
END $$;
...
BEGIN
  ...
  IF position('''service_interval_overdue''' IN v_src) = 0
    ...
    RAISE EXCEPTION '0448 V3: the visit promotion is not where the header says, or it keys on the incumbent''s trigger';
  END IF;
END $$;

-- ── LINEAGE ──
INSERT INTO public.ottoq_cert_lineage (name, forces_recert, note) VALUES
  ('0448_the_recall_policy_and_the_visit_planner_disagreed_about_maintenance_so_vehicles_looped',
   false,
...
-- ottoq_schema_snapshots label '0448_pre', set the experiment abandoned, delete implementation 3 and its function.
