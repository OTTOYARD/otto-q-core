...
--
-- EVIDENCE (live DB, run 4f9778de, 2026-08-23, completed, seed 3529985849887802304):
--   ottoq_decisions        redeployment + enacted  = 965 rows across 4 distinct vehicles
--                                                      (346 + 283 + 252 + 84)
--   ottoq_vehicle_commands dispatch               = 965 rows, issued_by='decide_tick', all
--                                                      status='executed'
--   ottoq_vehicle_dispatches                      = 91 rows, one per vehicle, status='completed'
--   => the decision ledger claims ~10x the deployments that actually happened.
--
-- TWO ROOT CAUSES, both in public.ottoq_decide_tick:
--
-- (1) NO IDEMPOTENCY ON THE REDEPLOY CANDIDATE QUERY. The cursor selects
--     current_state='staged_for_departure' vehicles and re-picks the same highest-SoC few every
--     tick because nothing marks them as "already decided for deploy, awaiting the twin." The
--     fix adds a guard: skip a vehicle that already carries an enacted redeployment decision at
--     or after its current state entry (last_state_change). last_state_change is only advanced by
--     an actual state change (trg_vehicle_state_change is guarded by
--     OLD.current_state IS DISTINCT FROM NEW.current_state), so the guard is one-decision-per-
--     staged-episode by construction, and it re-opens the vehicle after it actually deploys and
--     returns. Verified semantics, not assumed.
--
-- (2) THE 'dispatch' COMMAND IS A NO-OP. confirm_commands has no 'dispatch' verb (only
--     'deploy'/'proceed_to_gate' in its CASE, and those carry no state transition either); the
--     dispatch payload {'soc': N} has no stall_id, so it skips every executor branch and is
--     merely stamped 'executed'. Nothing in any schema consumes a 'dispatch' command row. The
--     real departure is twin.ottoq_sim_auto_dispatch_tick -> ottoq_plan_dispatch_tick (which
--     ranks by ottoq_brain_deploy_rank reading THIS decision) -> ottoq_sim_dispatch_vehicle.
--     The emission is removed; the decision row below remains the deploy signal.
--
-- NET EFFECT: redeployment decisions become one-per-deployable-episode (~91 for a 91-dispatch
-- run, matching reality) and the dead command rows stop accruing. The audit ledger stops lying.
--
-- TESTED: guard predicate verified against the live schema (sim_clock is timestamptz;
-- last_state_change is only advanced on a real state transition). A full live-run re-cert is
-- deferred to the apply step (production writes require founder approval).
    ...
    SELECT v.id AS vehicle_id, v.current_soc, v.fleet_operator_id
      FROM vehicles v WHERE v.home_depot_id=v_depot AND v.category='autonomous'
       ...
       AND v.current_soc < COALESCE((SELECT vn.target_soc FROM ottoq_visit_needs vn
              ...
              ORDER BY vn.created_at DESC LIMIT 1), public.ottoq_default_target_soc())
