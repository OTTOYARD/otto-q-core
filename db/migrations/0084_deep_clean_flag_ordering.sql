...
--
-- EVIDENCE (run 2e09ae5d, normal_day, seed 777008, vehicle c3333333-...0002):
--   profile.cabin_condition = 'soiled'
--   manifest output (rolled back) produced exterior_wash but NOT interior_deep_clean
--   because v_deep_clean_due was still false at the gate line (224) and set only at line 241.
--
-- FIX: move the v_deep_clean_due computation to immediately BEFORE the deep-clean gate, so the
-- flag is live when the IF reads it. No other logic changed; the 0082 wash backstop and the
-- 0083 condition itself are preserved.
