-- migration-version: 20260923061500
-- migration-name:    every_decision_in_the_stream_says_what_it_decided_and_why
--
-- 0455  **The Decisions stream printed the engine's name where the decision's reason belonged.** `db/checks/0356` §4.
--       FINDINGS G189.
--
--   For a decide-path row `ottoq_activity_feed` returns `rationale = proposed_action->'rationale'` and a `reason`
--   that falls through override codes, then that rationale as JSON text, then the first rule result, then
--   `l2_engine`. The decision's own `verb` and `reason` live beside the rationale, not inside it, and the feed
--   returned neither. Measured on run 736406cf: all 4,358 `stall_assignment` rows whose verdict was
--   `no_compatible_available_stall` reached the cockpit as reason "deterministic_v1"; 284 battery rows whose verdict
--   was `no_active_energy_plan_or_standby` likewise; and a charge assignment's purpose, need and stall type were
--   not in the row at all.
--
--   FIX: a decide-path row's `rationale` is its rationale object merged with its verdict -- verb, reason, purpose,
--   need, stall type, battery action, triage service, plan shift, override codes -- enacted values preferred over
--   proposed; and `reason` prefers the decision's own reason over the rationale blob and the engine's name. Agent
--   rows are unchanged. Same signature and columns; the ACL is kept. `forces_recert` FALSE: a read-only reporting
--   function that no tick path calls and no atom reads.
...
BEGIN
  ...
  IF v_pairs > 0 THEN RAISE EXCEPTION '0455 P0: a pair is running right now'; END IF;
END $inflight$;

-- ── P2: the 0454 body this extends ──
DO $premises$
BEGIN
  PERFORM 1 FROM pg_proc p WHERE p.oid = 'public.ottoq_activity_feed(uuid,integer,uuid,boolean,integer)'::regprocedure
     AND p.prosrc LIKE '%is_event%' AND p.prosrc LIKE '%ELSE d.proposed_action->''rationale'' END AS rationale%';
  IF NOT FOUND THEN RAISE EXCEPTION '0455 P2: ottoq_activity_feed is not the 0454 body this file extends'; END IF;
END $premises$;

INSERT INTO public.ottoq_schema_snapshots
       (label, object_kind, schema_name, object_name, definition, def_md5)
SELECT '0455_pre', 'function', n.nspname, p.proname, pg_get_functiondef(p.oid), md5(pg_get_functiondef(p.oid))
  FROM pg_proc p JOIN pg_namespace n ON n.oid = p.pronamespace
 WHERE p.oid = 'public.ottoq_activity_feed(uuid,integer,uuid,boolean,integer)'::regprocedure;

-- ═══ the stream, with verdicts ════════════════════════════════════════════════════════════════════════════
...
-- ═══ verification ═════════════════════════════════════════════════════════════════════════════════════════
DO $verify$
DECLARE v_run uuid; v_engine_named int; v_with_reason int; v_verbless_ts int; v_agent_changed int;
BEGIN
  IF NOT (has_function_privilege('anon', 'public.ottoq_activity_feed(uuid,integer,uuid,boolean,integer)', 'EXECUTE')
    ...
    RAISE EXCEPTION '0455 V1: the feed lost a grant';
  END IF;
  ...
  IF v_run IS NOT NULL THEN
    -- V2: a row whose decision carries its own reason shows that reason, never the engine's name
    SELECT count(*) FILTER (WHERE f.reason = 'deterministic_v1'),
           count(*) FILTER (WHERE f.rationale ? 'reason')
      INTO v_engine_named, v_with_reason
      FROM public.ottoq_activity_feed(v_run, 500, NULL, true, 240) f
     WHERE f.action <> 'orchestrator_agent' AND f.rationale ? 'reason';
    IF v_engine_named > 0 THEN
      RAISE EXCEPTION '0455 V2: % rows carry a reason and still print the engine name', v_engine_named;
    END IF;
    -- V3: every task_start row whose decision recorded a verb shows that verb
    SELECT count(*) INTO v_verbless_ts FROM public.ottoq_activity_feed(v_run, 500, NULL, true, 240) f
     WHERE f.action = 'task_start' AND NOT (f.rationale ? 'verb')
       AND EXISTS (SELECT 1 FROM public.ottoq_decisions d WHERE d.decision_seq = f.decision_seq
                    AND COALESCE(d.enacted_action->>'verb', d.proposed_action->>'verb') IS NOT NULL);
    IF v_verbless_ts > 0 THEN RAISE EXCEPTION '0455 V3: % task_start rows lost their verb', v_verbless_ts; END IF;
    -- V4: agent rows are untouched -- their rationale still carries the pipeline, not a verdict object
    SELECT count(*) INTO v_agent_changed FROM public.ottoq_activity_feed(v_run, 500, NULL, true, 240) f
     WHERE f.action = 'orchestrator_agent' AND (f.rationale ? 'verb' OR NOT (f.rationale ? 'solver_evidence'));
    IF v_agent_changed > 0 THEN RAISE EXCEPTION '0455 V4: % agent rows changed shape', v_agent_changed; END IF;
    RAISE NOTICE '0455 run %: % decide-path rows carry their own reason', v_run, v_with_reason;
  END IF;
END $verify$;

-- Rollback: restore the function from ottoq_schema_snapshots label '0455_pre' (CREATE OR REPLACE; the ACL is kept).

INSERT INTO public.ottoq_cert_lineage(name, forces_recert, note, classified_at)
VALUES ('0455_every_decision_in_the_stream_says_what_it_decided_and_why', false,
  'Reporting: ottoq_activity_feed returns each decide-path decision''s own verb and reason. No tick path reads it.', now())
