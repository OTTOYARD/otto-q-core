...
-- vehicle to 'ready' in the same tick, which pulled it back out of the bay, which interrupted the
-- booking, and the pair re-fired every tick. The vehicle oscillated without the wash ever running.
--
-- EVIDENCE (run 871cad10, vehicle 02ff42a9 / interior_deep_clean must-do):
--   0 transitions ever into in_wash_bay / in_detail_bay across the whole run
--   0 enter_wash / enter_service commands emitted anywhere in the run
--   28 wash bookings interrupted 'bay_exit_before_planned_end' (source=needs_card)
--   the same vehicle re-assigned to wash bay + promoted-ready every ~27s for 15+ min
--
-- TWO ROOT CAUSES, both in public.ottoq_decide_tick:
--  (1) Section (4b) emitted the weak 'proceed_to_stall' (no state transition) instead of
--      'enter_wash'/'enter_service' (which the twin's confirm_commands derives into
--      in_wash_bay/in_detail_bay/in_service_bay). Sections (4) and (5) already emit the strong
--      verbs for their own bays; (4b) was written inconsistently.
--  (2) Section (5)'s proposer (ottoq_l2_propose_service) only inspects service_bay atoms
--      (mechanical_pm/sensor_calibration/cosmetic_repair/fault_repair). A vehicle owing must-do
--      wash/detail work (exterior_wash/interior_deep_clean) looks "ready" to it, so it promoted
--      the vehicle the same tick (4b) was routing it to the wash lane.
--
-- FIX:
--  (1) (4b) now emits enter_wash (wash/detail) or enter_service (service), carrying booking_id,
--      leg_id and purpose so the twin seats the vehicle AND the calendar binds. This is the
--      "packet commits" fix -- the bay assignment now actually executes.
--  (2) (5) now skips vehicles that still owe must-do wash/detail work (they belong to (4b)). Once
--      the wash is done the vehicle returns to staged_awaiting_service and (5) promotes it then.
--
-- NET EFFECT: a multi-need vehicle charges (anchor) -> washes -> services -> deploys in order, with
-- each bay reservation actually binding, instead of oscillating. 0075 and 0080 guards preserved.
          ...
          SELECT c.vehicle_id, c.overall_urgency, c.must_do_now, c.minutes_to_deploy,
           ...
           WHERE c.depot_id = v_depot
             ...
             AND ( NOT ('charge' = ANY (COALESCE(c.must_do_now, '{}'::text[]))
                        OR COALESCE(c.soc_deficit_to_target, 0) > 0)
