...
-- among 'arrived_at_gate' vehicles: same states, same SoCs, same stall set in
-- use, different pairing. Two residual instances of already-fixed bug classes,
-- both in twin.ottoq_sim_auto_charge_assign_tick, plus the same salt bug in
-- twin.ottoq_sim_auto_dispatch_tick (caught by sweep, same pattern):
--
--   (1) ABSOLUTE-CLOCK RNG SALT (the 0045 class): v_seed hashed
--       p_sim_clock_now::text — the sim clock embeds the run's real start
--       time, so the seeded stall shuffle differed across same-seed runs.
--       Fixed by salting with twin.ottoq_sim_clock_salt(run, clock), the
--       0045 canonical domain (whole seconds since sim_clock_start).
--   (2) TIED-SoC HEAP ORDER (the 0050 class): the vehicle cursor was
--       ORDER BY current_soc ASC with no tiebreak; integer SoC ties (measured:
--       48/48 and 65/65 pairs at tick 10) left the claim order to heap order.
--       Fixed with the run-stable id tiebreak.
--
-- Bodies otherwise byte-identical to the 2026-08-19 captures in
-- db/fn_current/ (md5 2fff8e9d… / 5faaa92b… verified against production).
...
DECLARE
  ...
  v_seed       BIGINT;
BEGIN
  v_seed := abs(hashtextextended(COALESCE((SELECT random_seed::text FROM ottoq_sim_runs WHERE sim_run_id = p_sim_run_id), '42') || twin.ottoq_sim_clock_salt(p_sim_run_id, p_sim_clock_now) || 'assign', 13));   -- 0051: run-relative salt, never the absolute clock
  ...
  FOR v_vehicle IN
    SELECT id, current_soc, target_soc, fleet_operator_id, inlet_type, home_depot_id
     ...
     WHERE current_state = 'arrived_at_gate'
       ...
       AND current_soc < 85
     ORDER BY current_soc ASC, id                      -- lowest SOC first (most urgent); 0051: run-stable id tiebreak, never heap order
  LOOP
...
BEGIN
  ...
  v_seed := abs(hashtextextended(COALESCE(v_run.random_seed, 42)::text || twin.ottoq_sim_clock_salt(p_sim_run_id, p_sim_clock_now) || 'disp', 7));   -- 0051: run-relative salt, never the absolute clock
