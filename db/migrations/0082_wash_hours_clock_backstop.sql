...
-- emitted a wash atom on NIGHT-ROTATION (wash_group == sim_day % 3), SOIL override, or a CYCLES
-- backstop. A vehicle could be hours-overdue (card: must-do) yet not in tonight's rotation and
-- not yet at the cycles backstop, so no wash atom was ever materialized and it deployed unwashed.
--
-- EVIDENCE (run 87434f97, normal_day, seed 777006):
--   needs_card: 33 vehicles with wash/detail must-do; only 8 had a wash atom in visit_needs.
--   25 vehicles (76%) identified wash-due by the card never got an executable wash atom.
--   Section (4) returned 'skip_wash' 24x and 'admit_wash' 0x because no atom existed to admit.
--   Verified: last_wash_at IS advanced on wash completion (ottoq_wear_mark_serviced), so the
--   hours clock is real and working -- the manifest simply ignored it.
--
-- FIX: the manifest's wash gate gains a fourth trigger -- the HOURS-CLOCK overdue backstop. It
-- reads last_wash_at + wash_interval_h from vehicle_need_profile (the same source needs_card
-- grades by) and washes when the ratio reaches the exterior_wash overdue threshold. NULL-safe:
-- no profile row => no override; the existing rotation/soil/cycles gates are unchanged.
--
-- This makes the card and the manifest agree on wash by construction -- they read the same clock.
-- Overdue is a failure state, never a trigger: wash it on THIS arrival, don't wait for rotation.
...
BEGIN
  ...
  SELECT p.last_wash_at, p.wash_interval_h
    INTO v_last_wash, v_wash_int
  ...
  IF ottoq_sim_seeded_random(v_seed, v_salt || ':deep') < v_p THEN
