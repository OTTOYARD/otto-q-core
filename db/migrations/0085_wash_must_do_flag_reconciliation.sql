...
-- fired and the vehicle was promoted-to-ready AND routed-to-wash in the same tick, every tick.
--
-- EVIDENCE (run 661d0196, certified 143 sim-min, vehicle 6fdd7f85):
--   needs card: wash_urgency='overdue', must_do_now=[exterior_wash, software_update]
--   visit atom: exterior_wash must_do=FALSE (divergence)
--   decisions: needs_card assign_stall + deterministic_v1 promote_ready, same tick, every ~27s
--   7 bookings interrupted 'bay_exit_before_planned_end', all one vehicle
--   run-wide: 24 vehicles card-wash-overdue, 0 with a must_do=true wash atom
--
-- FIX: the wash atom's must_do flag is now driven by v_wash_overdue (the same hours-clock signal
-- the card grades wash_urgency by). Overdue -> must_do=true, deferrable=false; otherwise the
-- existing deferrable wash is unchanged. This makes the atom match the card, so the 0081 guard
-- fires and the vehicle is washed (not promoted) until the wash is done.
...
BEGIN
  ...
  -- Computed HERE, before the gate, so the flag is live when the IF below reads it.
  ...
  IF (v_is_night
    ...
    -- 0085: promote the atom to MUST-DO when the hours clock says overdue. 0082 made the
    -- manifest EMIT the atom on the hours backstop but left must_do=false, so the 0081 section-5
    -- guard (which skips vehicles with must-do wash atoms) never fired and the vehicle was
    -- promoted-to-ready AND washed in the same tick -- the churn 0081 was meant to kill. The
    -- card escalates wash to must-do at overdue_ratio; the atom now matches it.
