...
**CORRECTION 2026-09-22 (`db/checks/0329`) — THE SENTENCE ABOVE IS A LIVE CLAIM AND IS NOT RE-DERIVABLE FROM THE ARCHIVE, AND THE TWO ARE DIFFERENT CLAIMS.** Measured on the nine pairs certified 09:34–10:10 UTC: all nine recorded `equal=true` with `disagreeing_atoms = {}`, and **recomputing atom 4 (`events`) from the stored rows today makes all nine disagree.** In every pair arm A carries exactly one extra `vehicle.state_changed` event **per vehicle at that depot** (116 at the twin, 4 at the `grid_smoke` fixture), at arm A's final `sim_clock_at`, SoC-shaped with no `current_state` in the diff — while the transition population is identical (1,734 in both arms of the 48-tick pair). **Cause:** `ottoq_determinism_pair` runs both arms in ONE transaction and calls `ottoq_tick_invariance_reset_fleet` *before* the arm's run exists, while `ottoq.ottoq_active_sim_run_id()` caches its answer in a transaction-local GUC the pair never clears — so **arm 2's fleet reset is filed as arm 1's evidence**, stamped at arm 1's last tick.
   ...
   **AND IT HAPPENED AGAIN THE SAME DAY, ON THE SAME COLUMN, IN THE FLATTERING DIRECTION THIS TIME (`db/checks/0345`).** Re-censusing the callers with the literal `COALESCE(v_blocks,0)>0` found the honouring branch at **five** of `ottoq_decide_tick`'s **six** probe sites, and I was one sentence from reporting the decide path as a *mixed* caller that honours five and discards one. The sixth site spells it `IF COALESCE(v_blocks,0) > 0 THEN` — **spaces around the `>`.** It honours six of six: `bess_dispatch`, `redeployment`, `stall_assignment` and `task_start` ×3. **Fourth instance of one hazard, and the only thing that caught it was reading the site instead of trusting the count.** (Also: `twin.ottoq_sim_advance_visit_atoms` matches `ottoq_shield_probe` on a raw grep and **zero** times comment-stripped — it calls the wrapper. A caller census that does not strip comments overcounts.)
   **AND G149'S COST IS NOW QUANTIFIED, WHICH TURNS "SIX OF TEN CHECKPOINTS ARE ADVISORY" FROM AN ALARM INTO A WORK ORDER (`db/checks/0345`, G156).** Chase read `0337` and concluded *"if none of our rules work then we can't accurately calculate or determine anything."* **The first half is true of the mechanism and false of the protection, and the distinction is arithmetic, not rhetoric.** `ottoq_shield_probe` computes `would_block` as **`NOT passed AND enforcement='block'`**, so a `shadow`, `warn` or `log_only` verdict **cannot be a refusal** and discarding it is a no-op *by construction*. Across the six advisory contexts that leaves **exactly four (rule, context) pairs — and every one is a rule whose INPUT was broken, all four fixed on 2026-09-22**: HW.002 at `charge_session_start` (4,664 refusals, arithmetically could not pass, `0424`), HW.003 at `task_completion` (1,616, no jurisdiction, `0426`), HW.006 at `task_completion` (476, no legitimate input, `0427`), SM.006 at `bess_state_change` (15, misattributed actor, `0423`). **The two highest-traffic advisory contexts refuse NOTHING across 155,914 evaluations** — SM.001 fails 6,804 times at `vehicle_state_change` and not one is a refusal, because it is `shadow`. Windowed on each fix: **HW.002 3,702 evaluations / 0 failures, HW.003 3,362 / 0.** **SO THE DISCARD WAS MASKING BROKEN RULES, NOT MASKING DANGER** — which is also why `0337`'s own instruction (*"do not fix this by turning enforcement on"*) was right then and is no longer right now.
   **THE ONE GENUINELY OPEN SAFETY GAP, and it is the next promotion target.** Four energy rules fire at every charge start — `EN.001.grid_capacity_ceiling` and `EN.005.grid_event_hardstop` both `safety_critical`/`block`, plus `EN.002.stall_power_ceiling` and `EN.004.demand_response_compliance` — with **0 failures in 8,587 evaluations each, and the caller discards all four.** Today that costs nothing because the site power ceiling has never once been hit; **the day it is, the engine starts the charge anyway.** Promotion is cheap and this was measured rather than assumed: `twin.ottoq_sim_auto_charge_assign_tick` already wraps the call in `EXCEPTION WHEN OTHERS THEN CONTINUE`, so it already tolerates a refusal, and `twin.ottoq_sim_reconcile_charge_sessions` needs a three-line handler. **The order stays: fix the input, measure the rate to zero, promote ONE checkpoint, never six at once.**
   **AND THE HARD FLOOR IS NOT THE RULE LAYER AT ALL, which is what the "unread notes" framing cannot see.** What refuses unconditionally lives in the schema: `ottoq_stall_bookings`'s EXCLUDE constraint makes double-booking impossible, `HW.004.stall_single_vehicle` states in its own description that a partial unique index enforces it rather than the probe, and `ottoq_events` rejects every UPDATE and DELETE by trigger (`trg_ottoq_events_no_update`, read today) so the audit trail cannot be edited. Those do not write a note; they reject the write. **A rule census measures the advisory layer and is silent about the layer underneath it — quote both or neither.** And separately: **no atom reads a rule's enforcement.** `h_evt` digests `event_type|entity_id|sim_clock_at` and nothing else, so no KPI, capacity or determinism number in this repo depends on whether a verdict was honoured; what does depend on it is any sentence of the form *"the safety layer prevented N things"*, and that one stays unsayable until G149 closes.
   **G149 IS NOW CLOSED, AS A LEDGER FIX RATHER THAN AN ENFORCEMENT CHANGE (`db/migrations/0430`, applied `20260922172845`).** `public.ottoq_rule_evaluation_effect` puts what the SHIELD recommended beside what the ENGINE did: `blocked` at an enforcing caller reads **`refused`**, `blocked` at a discarding caller reads **`recorded_only`**, and a context with no resolvable caller reads `unknown_posture` rather than being quietly counted as either. `public.ottoq_shield_probe_posture()` derives the posture from the callers' own source **on every call** — comment-stripped, whitespace-tolerant and variable-name-tolerant, each of the three earned by a specific near-miss on this exact column — because a stored list was correct until `0422` added a probe point and `0428` promoted one, both the same day. **So the sentence is now a `WHERE` clause: `WHERE effect = 'recorded_only'`.** Verified at apply: **5 enforced, 5 advisory, 1 unresolvable**, blocked rows splitting **refused 7,371 / recorded_only 2,187 / unknown 0**, with base and view both 9,558 — it reclassifies and never filters.
   **AND THE CAVEAT IS 63% OF THE POPULATION, SO IT TRAVELS WITH EVERY FIGURE FROM THAT VIEW.** The posture is computed at **read** time, so a row is classified by today's code rather than by the code that wrote it: **4,664 of those 7,371 `refused` rows are `charge_session_start` rows written BEFORE `0428` promoted that checkpoint**, every one an `HW.002` false alarm from the defect `0424` fixed. The view is not wrong — it answers *"would today's engine refuse this"*, which is a different question from *"was this refused."* **Windowed on `0428` (`2026-09-22 17:01:57+00`) the honest split is `refused=106`, `recorded_only=32`, and that is the pair to quote.** This is `0334`'s standing rule arriving through a new door: normally it is the *data* that ages under a total, and here it is the *interpretation*.
   **The instrument also states where it is blind, which is the part to keep.** `public.ottoq_shield_and_log` passes the action context as a **variable** (`ottoq_shield_probe(v_ac, …)`) and *does* branch, so no static derivation can map it to a context; it is returned with `action_context IS NULL` and `posture='unresolvable_variable_context'` rather than omitted. Today that changes no answer — both contexts it serves are already enforced by `ottoq_decide_tick` — but a variable-context caller could one day be the only caller of some context, and a derivation that hid it would report that context as unprobed. **`0326` §6(a) in a new place: a static caller search can never find a rule's wiring, so the instrument must say so itself.**
   **AND THE TENTH PROBE POINT IS STRUCTURALLY BLIND TO EVERY SERVICE `HW.006` EXISTS TO JUDGE (`db/checks/0343`, G122 RE-OPENED).** `0425` and `0426` are both verified on fresh evidence, each matching its stated prediction — the walkaround now books **0** stalls beside `sensor_clean`'s 0 while `charge` (312) and `exterior_wash` (98) still book their own kinds, and HW.003 reads **224 evaluations, 0 failed, 224 abstained on its scope guard** (was 1,288 failures of 3,550). **But I predicted four of five codes vacuous and it is FIVE of five.** HW.006 also went to zero — from 234 — and the mechanism is not a fix: it reports *"insufficient context for presence verification"* with **zero** evaluations carrying a `stall_id`. **Removing the phantom booking removed the probe's only resolvable stall, and every stall it ever resolved was that phantom — so HW.006 at this probe point has never once had a legitimate input.** The cause is where `0422` spliced it: `twin.ottoq_sim_advance_visit_atoms` completes only the CONCURRENT classes (`cabin`/`exterior`/`digital`), so on the same runs **charge (242 completions), readiness_check (182), exterior_wash (88) and interior_deep_clean (28) complete with no `task_completion` evaluation at all** — 358 stall-occupying completions the rule never sees. **THE SENTENCE: *"all five declared codes at `task_completion` are vacuous there — and four of them are good rules asked the wrong question in the wrong place."*** 1,288 + 234 safety-critical false alarms are gone and **no protection was gained.** The remedy is a probe at the charge-session close and the service-flow bay exit, where `stall_id` is in hand by construction; `0418`'s probe already takes `p_stall_id`, so it is wiring, not a new instrument. **And note the shape against `0340`'s: that was a rule firing where it had no jurisdiction, this is a rule with jurisdiction that never sees a case. Both read as "evaluated" in any coverage census and neither protects anything — a probe point is not coverage until the population it sees can contain what the rule tests for.**
   **CLOSED AND VERIFIED 2026-09-22 by `db/migrations/0427` (applied `20260922163008`), and it paid for itself in seven minutes (`db/checks/0346`, G157).** The remedy `0343` prescribed — *"a probe at the charge-session close, where `stall_id` is in hand by construction"* — landed in `twin.ottoq_sim_stop_charge_session`, immediately **above** the tether-guarded `UPDATE stalls SET current_vehicle_id = NULL` (V1 asserts that ordering permanently; probing below it would have failed every untethered close and manufactured the very false-alarm class `0425`/`0426` had just removed 1,522 of). Measured on the window: **`HW.006` now carries a `stall_id` on 304 evaluations, against 0 for the entire prior life of the engine**, and `HW.003` sees **412 real charge cases with 0 failures** — so one probe moved two of `task_completion`'s five vacuous codes to meaningful, exactly as `0427` §2 predicted. **Note that `0343`'s stated remedy contained a false premise that `0427`'s own precondition caught before anything ran: the probe does NOT take a `p_stall_id`; it resolves the stall itself by `need_atom = p_svc`, which is why passing `p_svc := 'charge'` was the entire fix.**
   **AND HW.006'S FIRST REAL VERDICTS LAND ON `0427` §3'S FIRST PREDICTED BRANCH, WHICH MAKES THEM A FINDING RATHER THAN A SURPRISE: 16 failures, ALL SIXTEEN `l2`, against 96 of 96 passing on tethered `dcfc`.** The probe was placed above the clear precisely so that comparison would exist, and the split says the stall pointer is emptied *before* the charge-stop runs on **7.7% of untethered closes, by something that is not this function**. The evaluator's own payload separates two defects: **6 incidents where the pointer is EMPTY and 3 where it holds a DIFFERENT vehicle** (3 distinct others) — an L2 stall recording vehicle B while vehicle A's session closes on it. **The second is the serious half: it is not a calendar double-booking — the EXCLUDE constraint is not implicated — it is the physical pointer disagreeing with the session, which is what "assignment plus verification, always" and `space_conflict_ledger` exist to catch, and whether these incidents reached that ledger is not yet measured.** **HW.006 must STAY advisory on this evidence** — enforcing it today would refuse 7.7% of L2 charge completions, which is G150's lesson applied before the damage rather than after. **And it is NOT G121: adjacent is not identical.** G121 is three `dcfc` pointers too FULL and persistent across ticks; this is an `l2` pointer too EMPTY at one moment. `0333` made exactly that conflation in the other direction. What they share is a writer of `stalls.current_vehicle_id` the signed event stream does not explain (`0341`).
   **AND A COUNTING CAVEAT THAT MUST TRAVEL WITH EVERY NUMBER FROM THIS PROBE: among stall-carrying evaluations the rows are exactly 2.00 per `(stall, vehicle, evaluated_at)` — 396 rows over 198 triples — so "16 failures" is NINE incidents**, and anyone quoting the row count doubles the defect. I checked whether `0427` introduced the duplication, because assuming my own change caused it was the obvious first guess: **rows per triple is 5.05 before the apply and 3.30 after, so it predates the change and decreased.** Cause not established. `ottoq_probe_task_completion` takes no argument naming its caller — **the same gap G149 found in `enforcement_taken`: the ledger records what was decided and not who asked.**
